<div>
    ...
    <div class="flex flex-col justify-center bg-white rounded-xl">
        <div class="ms:w-90 md:w-90 border-b-2 border-[#faba67] flex">
            ...
            <div class="flex justify-between w-9/12 border-2 border-[#A9171C]">
                <div class="w-full flex flex-col justify-center border-2 border-[#F8981D">
                    <a href="{{ route('resource', $resource) }}">
                        ...
                        @if (Route::currentRouteName() == 'misreservas')
                            @foreach ($reservas as $reserva)
                                @if ($reserva->resource_id === $resource->id)
                                    <div class="w-full flex flex-row justify-between border-2 border-gray-900">
                                        <p class="italic p-2 font-thin text-left">{{ $reserva->date }} - {{ $reserva->name }}</p>
                                        {{-- <p class="italic p-2 font-thin">{{ $reserva->date }}</p>  --}}

                                        <button class="block text-white bg-[#F8981D] hover:bg-red-600 focus:ring-2 focus:outline-none focus:ring-orange-300 font-medium rounded-lg text-sm px-5 py-2.5 text-center dark:bg-[#F8981D] dark:hover:bg-red-600 duration-700 dark:focus:ring-orange-300 backdrop-grayscale backdrop-brightness-40" type="button" data-modal-toggle="defaultModal">Anular reserva</button>
                                    </div>

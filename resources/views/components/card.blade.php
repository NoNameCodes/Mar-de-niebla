        ...
        <div class="ms:w-90 md:w-90 border-b-2 border-[#faba67] flex">
            ...
            <div class="flex justify-between w-3/4">
                <div class="flex flex-col justify-center">
                    <a href="{{ route('resource', $resource) }}">
                        <p class="align-middle p-2 font-extrabold text-xl">{{ $resource->name }}</p></a>
                        @if (Route::currentRouteName() == 'misreservas')
                            ...
                            @endforeach
                       @else
                            <button type="button" onclick="location.href = '{{ route('history.id', $resource->id) }}'">Ver registro</button>
                        @endif
                ...
                <div class="flex flex-col justify-center p-6">
                    ...
                    <button class="btn inline-block select-none no-underline align-middle cursor-pointer whitespace-nowrap px-4 py-1.5 rounded text-base font-medium leading-6 tracking-tight text-white text-center border-0 bg-[#faba67] hover:bg-[#fa8e00] duration-300" type="button" onclick="location.href = '{{ route('reserva.destroy', $reserva->id) }}'">Anular Reserva</button>
                    @else
